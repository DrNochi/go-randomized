...
from dlgo.utils import keras
from dlgo.go import scoring
from dlgo.agents.neural.pg import load_policy_agent
from dlgo.go.__init__ import GameState, Player, Point
from dlgo.neural.rl.experience import combine_experience, ExperienceCollector, load_experience
...
def do_self_play(board_size, agent1_filename, agent2_filename,
    ...
    with h5py.File(agent1_filename, 'r') as agent1f:
        agent1 = load_policy_agent(agent1f)
    agent1.set_temperature(temperature)
    with h5py.File(agent2_filename, 'r') as agent2f:
        agent2 = load_policy_agent(agent2f)
    ...
    for i in range(num_games):
        ...
        if game_record.winner is color1:
            print('Agent 1 wins.')
...
def train_worker(learning_agent, output_file, experience,
                 lr, batch_size):
    with h5py.File(learning_agent, 'r') as learning_agentf:
        learning_agent = load_policy_agent(learning_agentf)

    for experience_file in experience:
...
def train_on_experience(learning_agent, output_file, experience,
                        lr, batch_size):
    worker = multiprocessing.Process(
        target=train_worker,
        args=[
...
def main():
    ...
    parser.add_argument('--games-in-memory', '-m', type=int, default=500)
    ...
    experience_file = os.path.join(args.work_dir, 'exp_temp_{}.hdf5')
    ...
    while True:
        ...
        experience = []
        for i in range(args.games_per_batch // args.games_in_memory):
            experience_part_file = experience_file.format(i)
            generate_experience(
                learning_agent, reference_agent,
                experience_part_file,
                num_games=args.games_in_memory,
                board_size=args.board_size,
                num_workers=args.num_workers,
                temperature=args.temperature)
            experience.append(experience_part_file)

        train_on_experience(
            learning_agent, tmp_agent, experience,
