...
class AlphaGoMCTS(Agent):
    ...
    def select_move(self, game_state):
        ...
        move = max(self.root.children, key=lambda m: self.root.children.get_color(m).visit_count)
    ...
    def policy_probabilities(self, game_state):
        ...
        legal_moves = game_state.possible_moves
        if not legal_moves:
            return [], []
        encoded_points = [encoder.encode_move(move.point) for move in legal_moves if move.point]
    ...
    def policy_rollout(self, game_state):
        ...
        if winner is not None:
            return 1 if winner is next_player else -1
        else:
            return 0
